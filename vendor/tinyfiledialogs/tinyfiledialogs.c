...
/* to be called from C# (you don't need this in C or C++) */
{
    ...
    else if (!strcmp(aIntVariableName, "tinyfd_allowCursesDialogs")) return
...
char* tinyfd_utf16to8(wchar_t const* aUtf16string)
{
    ...
    if (lSize)
    {
        ...
        lSize = WideCharToMultiByte(CP_UTF8, WC_ERR_INVALID_CHARS, aUtf16string, -1, lUtf8string,
                                    lSize, NULL, NULL);
    }
    ...
    wchar_t const* aIconType) /* L"info" L"warning" L"error" */
{
    ...
    if (quoteDetectedW(aTitle)) return tinyfd_notifyPopupW(L"INVALID TITLE WITH QUOTES", aMessage,
                                                           aIconType);
    if (quoteDetectedW(aMessage)) return tinyfd_notifyPopupW(
    ...
    wchar_t const* aDefaultInput) /* L"" , if NULL it's a passwordBox */
{
    ...
    if (quoteDetectedW(aTitle)) return tinyfd_inputBoxW(L"INVALID TITLE WITH QUOTES", aMessage,
                                                        aDefaultInput);
    if (quoteDetectedW(aMessage)) return tinyfd_inputBoxW(aTitle, L"INVALID MESSAGE WITH QUOTES",
                                                          aDefaultInput);
    if (quoteDetectedW(aDefaultInput)) return tinyfd_inputBoxW(
        aTitle, aMessage,
    ...
    int aDefaultButton) /* 0 for cancel/no , 1 for ok/yes , 2 for no in yesnocancel */
{
    ...
    if (tfd_quoteDetected(aTitle)) return tinyfd_messageBox("INVALID TITLE WITH QUOTES", aMessage,
                                                            aDialogType, aIconType, aDefaultButton);
    if (tfd_quoteDetected(aMessage)) return tinyfd_messageBox(
        aTitle, "INVALID MESSAGE WITH QUOTES", aDialogType, aIconType, aDefaultButton);
...
{
    if (tfd_quoteDetected(aTitle)) return tinyfd_notifyPopup(
        "INVALID TITLE WITH QUOTES", aMessage, aIconType);
    if (tfd_quoteDetected(aMessage)) return tinyfd_notifyPopup(
    ...
    char const* aDefaultInput) /* "" , if NULL it's a passwordBox */
{
    ...
    /* now I can fill lBuff from outside */

    if (tfd_quoteDetected(aTitle)) return tinyfd_inputBox("INVALID TITLE WITH QUOTES", aMessage,
                                                          aDefaultInput);
    if (tfd_quoteDetected(aMessage)) return tinyfd_inputBox(aTitle, "INVALID MESSAGE WITH QUOTES",
                                                            aDefaultInput);
    if (tfd_quoteDetected(aDefaultInput)) return tinyfd_inputBox(
        aTitle, aMessage, "INVALID DEFAULT_INPUT WITH QUOTES: use the GRAVE ACCENT \\x60 instead.");
    ...
    char const* aSingleFilterDescription) /* NULL or "image files" */
{
    ...
    if (tfd_quoteDetected(aTitle)) return tinyfd_saveFileDialog(
        "INVALID TITLE WITH QUOTES", aDefaultPathAndOrFile, aNumOfFilterPatterns, aFilterPatterns,
        aSingleFilterDescription);
    if (tfd_quoteDetected(aDefaultPathAndOrFile)) return tinyfd_saveFileDialog(
        aTitle, "INVALID DEFAULT_PATH WITH QUOTES", aNumOfFilterPatterns, aFilterPatterns,
        aSingleFilterDescription);
    if (tfd_quoteDetected(aSingleFilterDescription)) return tinyfd_saveFileDialog(
        aTitle, aDefaultPathAndOrFile, aNumOfFilterPatterns, aFilterPatterns,
        "INVALID FILTER_DESCRIPTION WITH QUOTES");
    for (i = 0; i < aNumOfFilterPatterns; i++)
    {
        if (tfd_quoteDetected(aFilterPatterns[i])) return tinyfd_saveFileDialog(
    ...
    else
    {
        ...
        /* preserve the current content of tinyfd_inputBox */
        p = tinyfd_inputBox(aTitle, lBuff, "");
        if (p) strcpy(lBuff, p);
        else lBuff[0] = '\0';
        if (lPointerInputBox) strcpy(lPointerInputBox, lString);
        /* restore its previous content to tinyfd_inputBox */
        p = lBuff;
    }
    ...
    int aAllowMultipleSelects) /* 0 or 1 */
{
    ...
    if (!aFilterPatterns) aNumOfFilterPatterns = 0;
    if (tfd_quoteDetected(aTitle)) return tinyfd_openFileDialog(
        "INVALID TITLE WITH QUOTES", aDefaultPathAndOrFile, aNumOfFilterPatterns, aFilterPatterns,
        aSingleFilterDescription, aAllowMultipleSelects);
    if (tfd_quoteDetected(aDefaultPathAndOrFile)) return tinyfd_openFileDialog(
        aTitle, "INVALID DEFAULT_PATH WITH QUOTES", aNumOfFilterPatterns, aFilterPatterns,
        aSingleFilterDescription, aAllowMultipleSelects);
    if (tfd_quoteDetected(aSingleFilterDescription)) return tinyfd_openFileDialog(
        aTitle, aDefaultPathAndOrFile, aNumOfFilterPatterns, aFilterPatterns,
        "INVALID FILTER_DESCRIPTION WITH QUOTES", aAllowMultipleSelects);
    for (i = 0; i < aNumOfFilterPatterns; i++)
    {
        if (tfd_quoteDetected(aFilterPatterns[i])) return tinyfd_openFileDialog(
            "INVALID FILTER_PATTERN WITH QUOTES: use the GRAVE ACCENT \\x60 instead.",
    ...
    char const* aDefaultPath) /* NULL or "" */
{
    ...
    if (tfd_quoteDetected(aTitle)) return tinyfd_selectFolderDialog(
        "INVALID TITLE WITH QUOTES", aDefaultPath);
    if (tfd_quoteDetected(aDefaultPath)) return tinyfd_selectFolderDialog(
    ...
    unsigned char aoResultRGB[3]) /* { 0 , 0 , 0 } */
{
    ...
    if (tfd_quoteDetected(aTitle)) return tinyfd_colorChooser(
        "INVALID TITLE WITH QUOTES", aDefaultHexRGB, aDefaultRGB, aoResultRGB);
    if (tfd_quoteDetected(aDefaultHexRGB)) return tinyfd_colorChooser(
        aTitle, "INVALID DEFAULT_HEX_RGB WITH QUOTES: use the GRAVE ACCENT \\x60 instead.",
...
static int dunstPresent(void)
{
    ...
    if (lDunstPresent < 0)
    {
        ...
        if (lDunstPresent)
        {
            lIn = (FILE*)popen("ps -e | grep dunst | grep -v grep", "r");
            /* add "| wc -l" to receive the number of lines */
            lTmp = fgets(lBuff, sizeof(lBuff), lIn);
            pclose(lIn);
...
int tfd_shellementaryPresent(void)
{
    ...
    if (lShellementaryPresent < 0)
    {
        ...
        /*detectPresence("shellementary"); shellementary is not ready yet */
    }
...
void tinyfd_beep(void)
{
    ...
    else if (speakertestPresent())
    {
        ...
        strcpy(lDialogString,
               "( speaker-test -t sine -f 440 > /dev/tty )& pid=$!;sleep .5; kill -9 $pid");
        /*.3 was too short for mac g3*/
    }
    ...
    int aDefaultButton) /* 0 for cancel/no , 1 for ok/yes , 2 for no in yesnocancel */
{
    ...
    if (osascriptPresent())
    {
        ...
        else if (aDialogType && !strcmp("yesnocancel", aDialogType))
        {
            ...
            {
            case 1: strcat(lDialogString, "default button \"Yes\" ");
                break;
            case 2: strcat(lDialogString, "default button \"No\" ");
                break;
            case 0: strcat(lDialogString, "default button \"Cancel\" ");
                break;
            }
...
-e 'else' -e 'return 0' -e 'end if' ");
        ...
        || tfd_qarmaPresent() || tfd_shantyPresent() || tfd_boxerPresent())
    {
        ...
        {
            if ((tfd_zenityVersion() >= 3900) || tfd_boxerPresent()) strcat(
                lDialogString, " --icon=dialog-");
        ...
        tkinter3Present())
    {
        ...
        else if (aDialogType && !strcmp("yesnocancel", aDialogType))
        {
            ...
            switch (aDefaultButton)
            {
            case 1: strcat(lDialogString, "default=messagebox.YES,");
                break;
            case 2: strcat(lDialogString, "default=messagebox.NO,");
                break;
            case 0: strcat(lDialogString, "default=messagebox.CANCEL,");
                break;
            }
        }
        else
...
frontmost of process \\\"Python\\\" to true' ''');");
        ...
        else if (aDialogType && !strcmp("yesnocancel", aDialogType))
        {
            ...
            switch (aDefaultButton)
            {
            case 1: strcat(lDialogString, "default=tkMessageBox.YES,");
                break;
            case 2: strcat(lDialogString, "default=tkMessageBox.NO,");
                break;
            case 0: strcat(lDialogString, "default=tkMessageBox.CANCEL,");
                break;
            }
        }
        else
...
if res is False :\n\tprint 0\n\
        ...
        xmessagePresent()))
    {
        ...
        if (aDialogType && !strcmp("okcancel", aDialogType))
        {
            ...
            switch (aDefaultButton)
            {
            case 1: strcat(lDialogString, " -default Ok");
                break;
            case 0: strcat(lDialogString, " -default Cancel");
                break;
            }
        }
        else if (aDialogType && !strcmp("yesno", aDialogType))
        {
            ...
            switch (aDefaultButton)
            {
            case 1: strcat(lDialogString, " -default Yes");
                break;
            case 0: strcat(lDialogString, " -default No");
                break;
            }
        }
        else if (aDialogType && !strcmp("yesnocancel", aDialogType))
        {
            ...
            case 1: strcat(lDialogString, " -default Yes");
                break;
            case 2: strcat(lDialogString, " -default No");
                break;
            case 0: strcat(lDialogString, " -default Cancel");
                break;
            }
...
to set mycolor to choose color default color {");
        ...
        || tfd_qarmaPresent() || tfd_shantyPresent() || tfd_boxerPresent())
    {
        ...
        if (tfd_zenityVersion() >= 2032 || tfd_matedialogPresent() || tfd_shellementaryPresent() ||
...
{
    *aiDefaultButton =
        tinyfd_messageBox(aTitle, aMessage, aDialogType, aIconType, *aiDefaultButton);
}
