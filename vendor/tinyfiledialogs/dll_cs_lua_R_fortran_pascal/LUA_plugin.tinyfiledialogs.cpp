...
static int GetFilters(lua_State* L, const char*** filters)
{
    ...
    if (lua_istable(L, -1))
    {
        ...
        if (n > STATIC_FILTER_COUNT) *filters = (const char**)lua_newuserdata(
            L, sizeof(const char*) * n); // ..., patts, filters
...
static luaL_Reg tfd_funcs[] = {
    ...
    },
    {
        "messageBox", [](lua_State* L)
        {
            ...
            lua_pushboolean(
                L, tinyfd_messageBox(title, message, dtype, itype, GetBool(L, "default_okyes")));
            // opts, dialog_type, icon_type, ok / yes
    ...
    },
    {
        "inputBox", [](lua_State* L)
        {
            ...
            return StringResponse(L, tinyfd_inputBox(title, message, def_input));
            // opts, def_input, input
        }
    },
    {
        "saveFileDialog", [](lua_State* L)
        {
            ...
            return StringResponse(
                L, tinyfd_saveFileDialog(title, def_path_and_file, nfilters, filters,
                                         filter_description)); // opts, patts[, filters], file
        }
    },
    {
        "openFileDialog", [](lua_State* L)
        {
            ...
            //
            const char* files = tinyfd_openFileDialog(title, def_path_and_file, nfilters,
                                                      nfilters ? filters : NULL, filter_description,
                                                      allow_multiple_selects);

            if (!allow_multiple_selects || !files) return StringResponse(L, files);
            // opts, patts[, filters], files?

            else
            {
                lua_newtable(L); // opts, patts[, filters], files

                char *from = (char*)files, *sep = from;
                // assign sep in order to pass first iteration
    ...
    },
    {
        "colorChooser", [](lua_State* L)
        {
            ...
            if (lua_istable(L, 3))
            {
                ...
                for (int i = 1; i <= 3; ++i) rgb[i - 1] = (unsigned char)(luaL_checknumber(L, 3 + i)
                    * 255.0);
            }
            ...
            {
                for (int i = 0; i < 3; ++i) lua_pushnumber(L, (double)rgb[i] / 255.0);
                // opts, out, rgb[, r, g, b], rout, gout, bout
